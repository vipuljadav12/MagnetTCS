...
class Submissions extends Model {
    ...
    public $fillable=[
        ...
        'gender',
        'letter_body',
        'holistic_committee_recommendation',
    ...
    public function scopegetSearhData(){
       ...
       $first_programs=Submissions::
        ...
        ->where('submissions.district_id', Session::get('district_id'))->where('submissions.enrollment_id', Session::get('enrollment_id'));
        ...
        if(!empty($allowed_programs))
            $first_programs = $first_programs->whereIn("first_choice_program_id", $allowed_programs);
        ...
        if(!empty($allowed_programs))
            $second_programs = $second_programs->whereIn("second_choice_program_id", $allowed_programs);
        ...
        $gender=Submissions::where('submissions.district_id', Session::get('district_id'))->where('submissions.enrollment_id', Session::get('enrollment_id'));
    ...
    public function scopegetSubmissionList($query,$all,$flag){
        ...
        if($all['gender']!=''){
            $query->where('submissions.gender',$all['gender'] );
        }
        ...
        if(!empty($allowed_programs))
        {
            $query->where(function($query) use($allowed_programs) {
                        $query->whereIn('submissions.first_choice_program_id', $allowed_programs)->orWhereIn('submissions.second_choice_program_id', $allowed_programs);
                });
        }
        if($all['search']['value']!=''){
        ...
        if($flag==1){
            return $query->orderBy('created_at','desc')->skip($all['start'])->take($all['length'])->where('submissions.enrollment_id', Session::get('enrollment_id'))->get();
