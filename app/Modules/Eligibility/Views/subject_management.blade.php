                                        ...
                                        <div id="{{$grade->name}}" aria-labelledby="heading{{$grade->name}}" data-parent="#accordionExample" class="collapse">
                                            <div class="card-body p-5 mt-20">
                                                ...
                                                @php 
                                                    ...
                                                    $yearDataArr = explode(',', $data['subjectManagement'][$grade->name]->year);
                                                    // dd($yearDataArr);
                                                @endphp
                                                <div class="row pl-10">
                                                    @forelse($academic_year_ary as $key=>$year)
                                                        <div class="col-md-3 mb-20">
                                                            <div class="row">
                                                                <div class="col-sm-6">
                                                                    {{$year}}
                                                                ...
                                                                <div class="col-sm-6">
                                                                    {{-- @php $subject=strtolower(str_replace(' ','_',$subject)) @endphp --}}
                                                                    {{-- /*!empty($data['subjectManagement']->where('grade_id',$grade->id)->where($subject,'Y')->first())*/ --}}
                                                                    {{-- <input type="hidden" name="gradeYear[{{$grade->name}}][{{$year}}]" value="N"> --}}
                                                                    <input type="checkbox" value="{{$year}}" name="gradeYear[{{$grade->name}}][{{$year}}]" class="js-switch js-switch-1 js-switch-xs status" data-size="Small" @if(in_array($year, $yearDataArr)) checked @endif />
                                                                </div>
                                                            </div>
                                                        </div>
                                                    @empty
