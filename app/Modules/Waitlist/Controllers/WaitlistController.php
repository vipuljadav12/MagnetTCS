            ...
            */
            ...
            while (!empty($hold_arr) && $hld_cnt != $actual_hld_cnt) { // 6
                ...
                foreach ($sortedArray as $skey => $svalue) { // 5
                    ...
                    $new_percent = number_format(($exist_count * 100) / $adm_value, 2);
                    //dd($this->availabilityArray);
                    if ($new_percent >= 7 && in_array($program_id, $program_keys)) { //4

                        if ($this->availabilityArray[$program_id][$next_grade] > 0) { //3
                            $removedId = [];
                            $offered = false;
                            foreach ($hold_arr as $key => $value) { // 2
                                if ($program_id == $value['program_id'] && $next_grade == $value['next_grade'] && $school_id == $value['school_id']) { //1
                                    /*if(!in_array($tmp['id'], $this->offered_ids))
                                        {*/
                                    $avlQty = $this->availabilityArray[$value['program_id']][$value['next_grade']];
                                    $avlQty--;
