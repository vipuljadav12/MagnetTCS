                        ...
                        @isset($disp_arr)
                            ...
                            @foreach($disp_arr as $key=>$value)
                                ...
                                @foreach($value as $wkey=>$wvalue)
                                    ...
                                    <tr class="{{$class}}" id="row{{$wvalue['application_program_id']}}">
                                        ...
                                        <td class="text-center">
                                            <span class="{{$spanclass}}">
                                                <select class="form-control withdrawn_student" data-application_program_id="{{$wvalue['application_program_id']}}" style="width:100px;" onchange="enableDisableWithdrawn({{$count}}, $(this).val())" name="withdrawn_student{{$wvalue['application_program_id']}}">
                                    ...
                                    @if(!empty($wvalue['homezone_schools']))
                                        @php
                                            $rowspan = ceil((float)(count($wvalue['homezone_schools'])/3));
                                        @endphp
                                        @foreach(array_chunk($wvalue['homezone_schools'], 3) as $hzs_key=>$hzs_chunk)
                                            <tr class="hzs_container_{{$wvalue['application_program_id']}}">
                                                @if($hzs_key == 0)
                                                    <td colspan="4" rowspan="{{$rowspan}}"></td>
                                                @endif
                                                @foreach($hzs_chunk as $hzs_name)
                                                    <td>
                                                        {{$hzs_name}}{{-- ||{{$rowspan}}||{{count($wvalue['homezone_schools'])}} --}}<br>
                                                        <input type="text" name="homezone_schools[{{$wvalue['application_program_id']}}][{{getSchoolName($hzs_name)}}]" class="form-control text-center hzs_val" onkeypress="return onlyNumberKey(event)" style="width: 100px;" value="{{($wvalue['homezone_val'][getSchoolName($hzs_name)] ?? 0)}}">
                                                    </td>
                                                @endforeach
                                                @if(count($hzs_chunk) != 3)
                                                    @for($tmp_i=0; $tmp_i<(3-count($hzs_chunk)); $tmp_i++)
                                                        <td></td>
                                                    @endfor
                                                @endif
                                                @if($hzs_key == 0)
