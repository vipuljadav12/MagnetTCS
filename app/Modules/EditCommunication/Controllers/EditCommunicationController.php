...
class EditCommunicationController extends Controller
{
  ...
  public function storeLetter(Request $request){
    ...
    public function generate_letter_now($status, $application_id, $preview=false)
    {
        ...
        $application_data = Application::join("enrollments", "enrollments.id", "application.enrollment_id")->where('application.district_id', Session::get('district_id'))->where("application.status", "Y")->select("application.*", "enrollments.school_year")->where("application.id", $application_id)->first();
        ...
        $rs = ProcessSelection::where("application_id", $application_id)->first();
        ...
        if($preview == true)
        {
            ...
            $tmp['parent_address'] = "mark.william@gmail.com";
            $tmp['student_id'] = "1234567890";
  ...
  public function storeEmail(Request $request){
    ...
    {
        $cdata = EditCommunication::where("status", $status)->where("application_id", $application_id)->first();
        $application_data = Application::join("enrollments", "enrollments.id", "application.enrollment_id")->where('application.district_id', Session::get('district_id'))->where("application.id", $application_id)->select("application.*", "enrollments.school_year")->first();
        $district_id = Session::get('district_id');
