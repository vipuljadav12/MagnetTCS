 <script type="text/javascript">
    ...
    $("#academic_grades").change(function(){
        if($(this).prop("checked") == true)
            $("#academic_grades_options").removeClass("d-none");
        else
            $("#academic_grades_options").addClass("d-none");
    })

    $("#conduct_discpline").change(function(){
        if($(this).prop("checked") == true)
            $("#conduct_criteria").removeClass("d-none");
        else
            $("#conduct_criteria").addClass("d-none");
    })
    
    $("#range_selection_1").change(function(){
        if($(this).val() == "abc")
        {
            $("#abc_criteria_1").removeClass("d-none");
            $("#3s2s_criteria_1").addClass("d-none");
        }
        else if($(this).val() == "3s2s")
        {
            $("#abc_criteria_1").addClass("d-none");
            $("#3s2s_criteria_1").removeClass("d-none");
        }
    })
    ...
    $("#range_selection_2").change(function(){
        if($(this).val() == "abc")
        {
            $("#abc_criteria_2").removeClass("d-none");
            $("#3s2s_criteria_2").addClass("d-none");
        }
        else if($(this).val() == "3s2s")
        {
            $("#abc_criteria_2").addClass("d-none");
            $("#3s2s_criteria_2").removeClass("d-none");
        }
    })

    /* LFD Admin start */
    $(document).on('change', '.chk_rec_teachers[value="lfd_admin"]', function() {
        lfdAdminFields($(this));
    });
    $(document).find('.chk_rec_teachers[value="lfd_admin"]').each(function(idx, elm) {
        lfdAdminFields($(this));
    });
    // lfdAdminFields();
    function lfdAdminFields(e) {
        let lfd_chk = e;
        let is_lfd_admin_checked = lfd_chk.prop('checked');
        let field_container = lfd_chk.closest('.form-group').find('.fld_for_lfd_admin');
        if (is_lfd_admin_checked) {
            field_container.removeClass('d-none');
            field_container.find('input').prop('disabled', false);
        } else {
            field_container.addClass('d-none');
            field_container.find('input').prop('disabled', true);
        }
    }
    /* LFD Admin end */
    /* Recommendation Form start */
    $(document).on('change', '.ayg_parent_chk', function() {
        let is_checked = $(this).prop('checked');
        if (!is_checked) {
            let ayg_container = $(this).closest('.ayg_container');
            let ayg_child_chk_container = ayg_container.find('.ayg_child_chk_container');
            ayg_child_chk_container.find('.ayg_child_chk').prop('checked', false);
        }
    });
    /* Recommendation Form end */
</script>