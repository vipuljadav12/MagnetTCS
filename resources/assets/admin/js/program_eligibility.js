...
function showGradePopup(eligibility_id, name, option_val="")
{
	//For late submission
	var extra_val = '';
	...
	$('#modal_1').modal('show');
	var values = $("#grade"+extra_val+eligibility_id+option_val).val();
	// console.log(values);
	var gradeArr = values.split("-");
	$(".eligibility_type_name").html(name);
	$(".gradeClass").each(function(){
		$(this).attr("name", "eligibility_grade_lavel["+eligibility_id+"]["+option_val+"[]");
