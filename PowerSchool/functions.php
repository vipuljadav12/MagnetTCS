...
function getPowerSchoolRecords($type, $accessTokenKey, $url, $sdata) {
    ...
    switch ($type) {
        ...
        break;
        case 'student_standardgradesection':
            $queryName = 'org.magnet_tcs.standardgradesection.hs_student_standardgradesection';
            break;
        case 'sectionteacher':
    ...
    if (!empty($type) && $type == "student_standardgradesection") {

         $resource = $url . '/ws/schema/query/' . $queryName.'?pagesize=0';
        $payload = '{}';
        if ($type == 'student_standardgradesection') {
        ...
        // $fields = array_keys($jsonData['record'][0]['tables']['standardgradesection']);
        ...
        //     $tmp = $item['tables']['standardgradesection'];
        //     foreach($fields as $val)
        //     {
        //         echo $tmp[$val]."^";
        //     }
        ...
        if (isset($jsonData['record'])) {
            foreach ($jsonData['record'] as $item) {
                $data  = $item['tables']['standardgradesection'];
                $yr = $data['yearid'];
                if($yr == 33)
                {
                    if($data['standardgrade'] != '' && is_numeric($data['standardgrade']))
                    {
                        $academicYear = (1990+$yr) ."-".((1990+$yr+1)-2000);
                        $term = $data['storecode'];
                        $numericGrade = $data['standardgrade'];
                        $submission_id = $sdata['submission_id'];
                        $student_id = $sdata['student_id'];

                        $sql = "SELECT subjectarea, identifier FROM ps_standard WHERE standardid = '".$data['standardid']."'";
                        $rsNew = $objDB->select($sql);
                        if(count($rsNew) > 0)
                        {
                            if(in_array($rsNew[0]['subjectarea'], ['Social Studies', 'SS', 'Science', 'SC', 'Engilsh Language Arts', 'EN', 'Mathematics', 'MA']))
                            {                                if(in_array($rsNew[0]['subjectarea'], ['Social Studies', 'SS']))
                                {
                                    $insert['courseType'] = $insert['courseTypeID'] ='SS';
                                    $insert['courseName'] = 'Social Studies';
                                    $insert['standard_identifier'] = $rsNew[0]['identifier'];
                                }
                                ...
                                if(in_array($rsNew[0]['subjectarea'], ['Science', 'SC']))
                                {
                                    $insert['courseType'] = $insert['courseTypeID'] ='SC';
                                    $insert['courseName'] = 'Science';
                                }
                                if(in_array($rsNew[0]['subjectarea'], ['English Language Arts', 'EN']))
                                {
                                    $insert['courseType'] = $insert['courseTypeID'] ='EN';
                                    $insert['courseName'] = 'English';
                                }
                                if(in_array($rsNew[0]['subjectarea'], ['Mathematics', 'MA']))
                                {
                                    $insert['courseType'] = $insert['courseTypeID'] ='MA';
                                    $insert['courseName'] = 'Math';
                                }
                                $insert['standard_identifier'] = $rsNew[0]['identifier'];
                                
                                $insert['submission_id'] = $sdata['submission_id'];
                                $insert['stateID'] = $sdata['student_id'];
                                $insert['academicYear'] = $academicYear;
                                $insert['GradeName'] = $insert['academicTerm'] = str_replace(" Grade", "", $term)." Grade";
                                $insert['numericGrade'] = $insert['actual_numeric_grade'] = $numericGrade;
                                $insert['grade'] = $data['gradelevel'];
                                $insert['submission_id'] = $submission_id;

                                 if(isset($insert['numericGrade']) && $insert['numericGrade'] != '' && in_array($insert['academicTerm'], array('Q1 Grade', 'Q2 Grade')) && $insert['courseTypeID'] != '')
                                {       

                                    $SQL = "DELETE FROM submission_grade WHERE submission_id = '".$insert['submission_id']."' AND courseType = '".(isset($insert['courseType']) ? $insert['courseType'] : "")."' AND academicYear = '".$insert['academicYear']."' AND academicTerm = '".$insert['academicTerm']."' AND standard_identifier = '".$insert['standard_identifier']."'";
...
//        $result = file_get_contents($resource, false, stream_context_create($opts));
        //Get the JSON data
        $jsonData = json_decode($result, true);
     //   echo base64_decode('');exit;
...
//print_r($jsonData);exit;
        ...
        if (isset($jsonData['record'])) {
            foreach ($jsonData['record'] as $item) {
                ...
                $academicYear = number_format($term/100, 0) + 1990;
                $yrid = $academicYear;// ."-".($academicYear+1);
                
                if($yrid != '' && in_array($yrid, array(2023)))
                {
                    ...
                    if($item['tables']['storedgrades']['credit_type'] == 'C,MTH' || strpos(strtolower($item['tables']['storedgrades']['course_name']), 'math') !== false || strpos(strtolower($item['tables']['storedgrades']['course_name']), 'algebra') !== false || strpos(strtolower($item['tables']['storedgrades']['course_name']), 'geometry') !== false  || strpos(strtolower($item['tables']['storedgrades']['course_name']), 'precalculus') !== false){
                        $gdata['courseTypeID'] = $gdata['courseType'] = 'MA';//4;
                        $gdata['courseFullName'] = $gdata['courseName'] = "Math";
                        ...
                        $gdata['courseTypeID'] = $gdata['courseType']  = 'SS';//9;
                        $gdata['courseFullName'] = $gdata['courseName'] = "Social Studies";
                    }
                    else if(strpos(strtolower($item['tables']['storedgrades']['course_name']), 'reading') !== false) {
                        $gdata['courseTypeID'] = $gdata['courseType']  = 'RE';//0;
                        $gdata['courseFullName'] = $gdata['courseName'] = "Reading";
                    }
                        ...
                        $gdata['courseTypeID'] = $gdata['courseType']  = 'SC';//7;
                        $gdata['courseFullName'] = $gdata['courseName'] = "Science";
                    }
                    else if($item['tables']['storedgrades']['credit_type'] == "C,ELA"  || strpos(strtolower($item['tables']['storedgrades']['course_name']), 'english') !== false) {
                        $gdata['courseTypeID'] = $gdata['courseType'] = 'EN';//3;
                        $gdata['courseFullName'] = $gdata['courseName'] = "English";
                    }
                    ...
                    if(isset($gdata['numericGrade']) && $gdata['numericGrade'] != '' && in_array($gdata['academicTerm'], array('Q1 Grade', 'Q2 Grade')) && $gdata['courseTypeID'] != '')
                    {       
